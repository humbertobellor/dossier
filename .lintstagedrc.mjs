...
export default (stagedFiles) => {
  ...
  const fontRelated = stagedFiles.filter((f) => f.endsWith(".css") || f.includes("/public/fonts/"));

  if (fontRelated.length > 0) {
    commands.push(`pnpm --filter @workspace/scripts run check-fonts -- ${fontRelated.join(" ")}`);
  }
